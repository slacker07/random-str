...
class UnrecognizedArgument extends Error {
    constructor(message) {
        super(message);
...
module.exports = function generate(type = 'string', length = 1, uppercase = false) {
    try {
        ...
        if (length > 1e6 || length < 1) {
            throw new RangeError(chalk.bold(`Length range out of boundry (Max ${1e6}, Min 1)`));
        }
        ...
        throw new UnrecognizedArgument(`unrecognized argument "${type}"`)
    } catch (error) {
        // throw new Error(`${error}`)
        const message = `--= ${chalk.yellow(chalk.bold(error.name))} :: ${chalk.red(error.message)} =-- \n${error.stack}`
        return message;
    }
}
